  ...
  <?php
    ...
    if(isset($_POST['postComment' . $post_id])) {
      ...
      if ($posted_to != $userLoggedIn) {
        ...
        $notification->insert_notification($post_id, $posted_to, "comment");
      }

      if ($user_to != 'none' && $user_to != $userLoggedIn) {
        $notification = new Notification($connection, $userLoggedIn);
        $notification->insert_notification($post_id, $posted_to, "profile_comment");
      }

      $get_comments = mysqli_query($connection, "SELECT * FROM comments WHERE post_id='$post_id'");
      ...
      while($row = mysqli_fetch_array($get_comments)) {
        if($row['posted_by'] != $posted_to
          ...
          $notification = new Notification($connection, $userLoggedIn);
          $notification->insert_notification($post_id, $row['posted_by'] , "comment_non_owner");
   ...
   <?php
    ...
    if($count != 0) {
      ...
      $user_object = new User($connection, $posted_by);
  ?>
      <div class="comment_section">
        <a href="<?php echo $posted_by; ?>" target="_parent">
          <img src="<?php echo $user_object->get_profile_picture(); ?>" title="<?php echo $posted_by; ?>"
            style="float: left; height: 30px;">
        </a>
        <a href="<?php echo $posted_by; ?>" target="_parent">
          <b><?php echo $user_object->get_first_and_lastname(); ?></b>
        </a>
